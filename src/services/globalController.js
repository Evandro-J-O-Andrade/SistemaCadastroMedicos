    ...
    <div className="p-4 max-w-5xl mx-auto">
          ...
          <tbody>
            {records.map((r, idx) => (
              <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                ...
                <td className="p-2 align-top"><input className="w-full border rounded p-1" value={r.crm || (r.medico && r.medico.crm) || ""} onChange={e=>handleCellChange(idx,"crm",e.target.value)}/></td>
                ...
                <td className="p-2 align-top"><input className="w-full border rounded p-1" value={r.data || r.date || ""} onChange={e=>handleCellChange(idx,"data",e.target.value)}/></td>
                <td className="p-2 align-top"><input className="w-full border rounded p-1" value={r.hora || r.time || ""} onChange={e=>handleCellChange(idx,"hora",e.target.value)}/></td>
      ...
      <div className="mt-8 text-sm text-gray-600">
        ...
        <ul className="list-disc pl-5">
          <li>Se nenhum filtro for passado, a busca traz todos os plantões.</li>
          <li>Se passar nome do médico + especialidade, o sistema valida se o médico tem a especialidade no cadastro.</li>
          <li>Campos da tabela são editáveis; clique em "Salvar edições" para persistir no localStorage.</li>
        </ul>
      </div>
    </div>
  );
