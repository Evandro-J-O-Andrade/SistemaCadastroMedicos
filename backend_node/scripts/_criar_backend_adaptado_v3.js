// criar_backend_adaptado_v3.js — PARTE 1
...
const folders = [
  ...
  "logs",
  "config",
  "scripts" // helper scripts se necessário
];
...
/* --------------------------
 .env (exemplo)  <- estamos usando .env conforme pedido
---------------------------*/
fs.writeFileSync(
...
NODEMAILER_PORT=587
NODEMAILER_USER=darkangelyas.yash@gmail.com
NODEMAILER_PASS=ztzx wifg ykhz bxyc
ADMIN_EMAIL=darkangelyas.yash@gmail.com
...
// rotas (serão criadas pelo script — parte 2/3 terá as rotas completas)
...
// middleware de auditoria leve (não bloqueante)
app.use(logRequest);
...
/* --------------------------
 utils/database.js (um helper sqlite simples, usado se preferir em vez de open())
---------------------------*/
fs.writeFileSync(
...
/* --------------------------
 utils/apiConfig.js
  — configuração compartilhada que pode ser utilizada pelo front/back (geralmente .env)
---------------------------*/
fs.writeFileSync(
  path.join(backendDir, "utils", "apiConfig.js"),
  `// utils/apiConfig.js
// Pequeno helper para expor valores de config (somente leitura).
import dotenv from "dotenv";
dotenv.config();

export const API_CONFIG = {
  API_URL: process.env.FRONTEND_URL || "http://localhost:5173",
  BACKEND_URL: \`http://localhost:\${process.env.PORT || 5000}\`,
...
/* --------------------------
 utils/apiFetch_example.js
  — helper de cliente para usar no front (exemplo; pode ser copiado para frontend)
---------------------------*/
fs.writeFileSync(
  path.join(backendDir, "utils", "apiFetch_example.js"),
  `// utils/apiFetch_example.js
...
// middleware não-bloqueante que grava rota/metodo/ip/payload
export async function logRequest(req, res, next) {
  try {
    const db = await open({ filename: DB_FILE, driver: sqlite3.Database });
    const user = req.user ? req.user.username : null;
    // grava sem await excessivo para não bloquear (mas usamos await para garantir escrita)
    await db.run(
...
/* --------------------------
 Mensagem final parte 1
---------------------------*/
console.log("✅ PARTE 1: arquivos básicos (package.json, .env, server, utils, middleware, db init/seed) criados em /backend_node");
console.log("Execute:");
console.log("  cd backend_node");
console.log("  npm install");
console.log("  npm run init-db");
console.log("  npm run seed");
console.log("Depois eu envio a PARTE 2 com todas as rotas completas (auth, usuarios, medicos, plantoes, especialidades, atendimentos, relatorios, status) e testes.");
/* ============================
   PARTE 2 — rotas + utils extras
   Cole em seguir a PARTE 1 no mesmo arquivo criar_backend_adaptado_v3.js
   ============================ */
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res.json({ user: { id: user.id, username: user.username, tipo: user.tipo, email: user.email }, token });
  } catch (e) {
    console.error(e);
...
router.post("/recuperar-senha", async (req, res) => {
  ...
  try {
    ...
    const user = await db.get("SELECT id, email FROM usuarios WHERE username = ?", [username.toLowerCase()]);
    if (!user) return res.status(404).json({ error: "Username não encontrado" });
    // token curto que o admin usará para resetar
    const token = jwt.sign({ id: user.id, action: "recover" }, JWT_SECRET, { expiresIn: "1h" });
    // envia email para o admin com info do usuário e token — utils/emailService faz o envio
    await sendRecoveryToAdmin(username.toLowerCase(), token, user.email || null);
...
// Reset de senha pelo admin (rota pública que recebe token)
router.post("/reset-senha/:token", async (req, res) => {
  const { senha } = req.body;
  const { token } = req.params;
  try {
    if (!senha) return res.status(400).json({ error: "nova senha obrigatória" });
...
// lista usuários (apenas admin/suporte)
router.get("/", autenticarToken, autorizarPerfis("admin", "suporte"), async (req, res) => {
  const db = await open({ filename: DB_FILE, driver: sqlite3.Database });
  try {
    const users = await db.all("SELECT id, username, email, tipo, primeiro_login, criado_em, atualizado_em FROM usuarios ORDER BY criado_em DESC");
    res.json(users);
  } catch (e) {
    console.error(e);
...
router.post("/", autenticarToken, autorizarPerfis("admin"), validate(schemas.usuario), async (req, res) => {
  ...
  try {
    ...
    const r = await db.run("INSERT INTO usuarios (username, email, senha, tipo, criado_em) VALUES (?, ?, ?, ?, datetime('now'))", [username.toLowerCase(), email || null, hash, tipo || "usuario"]);
    res.status(201).json({ id: r.lastID, msg: "Usuário criado" });
  } catch (e) {
    console.error(e);
...
/* --------------------------
 routes/medicos.js (completo — inclui /pesquisa)
---------------------------*/
fs.writeFileSync(
...
router.get("/:id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    res.json(medico);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro ao buscar médico" }); } finally { await db.close(); }
});
...
router.get("/pesquisa", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro na pesquisa" }); } finally { await db.close(); }
});
...
router.get("/", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    const rows = await db.all(sql);
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro listar plantoes" }); } finally { await db.close(); }
});
...
router.get("/filtro", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro filtro" }); } finally { await db.close(); }
});
...
router.post("/", autenticarToken, autorizarPerfis("admin", "suporte"), validate(schemas.plantao), async (req, res) => {
  ...
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro criar plantao" }); } finally { await db.close(); }
});
...
router.put("/:id", autenticarToken, autorizarPerfis("admin", "suporte"), async (req, res) => {
  ...
  try {
    ...
    res.json({ sucesso: true });
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro atualizar" }); } finally { await db.close(); }
});
...
router.delete("/:id", autenticarToken, autorizarPerfis("admin", "suporte"), async (req, res) => {
  ...
  try {
    ...
    if (r.changes === 0) return res.status(404).json({ error: "Plantão não encontrado" });
    res.json({ sucesso: true });
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro excluir" }); } finally { await db.close(); }
});
...
router.post("/relatorio", autenticarToken, autorizarPerfis("admin", "suporte"), async (req, res) => {
  ...
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro relatorio" }); } finally { await db.close(); }
});
...
router.get("/", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro listar" }); } finally { await db.close(); }
});
...
router.post("/", autenticarToken, autorizarPerfis("admin"), validate(schemas.especialidade), async (req, res) => {
  ...
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro criar" }); } finally { await db.close(); }
});
...
router.get("/:plantao_id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro listar atendimentos" }); } finally { await db.close(); }
});
...
router.post("/", autenticarToken, autorizarPerfis("admin", "suporte"), async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: r.lastID });
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro criar atendimento" }); } finally { await db.close(); }
});
...
router.put("/:id", autenticarToken, autorizarPerfis("admin", "suporte"), async (req, res) => {
  ...
  try {
    ...
    res.json({ msg: "Atualizado" });
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro atualizar atendimento" }); } finally { await db.close(); }
});
...
router.get("/plantao", async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro gerar relatório" }); } finally { await db.close(); }
});
...
router.get("/data", async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro gerar relatório por data" }); } finally { await db.close(); }
});
...
router.get("/atendimentos", async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Erro gerar relatório filtrado" }); } finally { await db.close(); }
});
...
/* --------------------------
 utils/apiConfig.js  (pequeno helper / config export)
---------------------------*/
fs.writeFileSync(
  path.join(backendDir, "utils", "apiConfig.js"),
...
/* --------------------------
 utils/apiFetch.js (helper fetch para uso interno se necessário)
---------------------------*/
fs.writeFileSync(
  path.join(backendDir, "utils", "apiFetch.js"),
...
/* --------------------------
 utils/README_API_CLIENT.md (instruções rápidas para front)
---------------------------*/
fs.writeFileSync(
  path.join(backendDir, "utils", "README_API_CLIENT.md"),
...
/* --------------------------
 Mensagem final da PARTE 2
---------------------------*/
console.log("✅ PARTE 2 gravada: rotas + utils extras criadas em /backend_node (auth, usuarios, medicos, plantoes, especialidades, atendimentos, relatorios, status, apiConfig, apiFetch, README).");
console.log("Rode dentro de /backend_node:");
console.log("  npm install");
console.log("  npm run init-db");
console.log("  npm run seed");
console.log("  npm run dev");
console.log("A seguir, prossiga para a PARTE 3 para criar o frontend em React.");